 ...
 *	double (* f) (double) f		The function under consideration
 ...
 *
 * $Id: zeroin.cc,v 4.0 1997/06/11 18:33:09 oleg Exp oleg $
 *
 ************************************************************************
 */
		...
		const double tol)		// Acceptable tolerance
{
	...
	for(;;)		// Main iteration loop
	{
		...
		const double tol_act = 2*std::numeric_limits<double>::epsilon()*abs(b) + (tol>0?tol:0.0)/2;
		...
		{					// Interpolatiom may be tried
			...
			double q;  // lated in the form p/q; division operations is delayed
		...
		a = b;
		fa = fb;			// Save the previous approximation
		b += new_step;
