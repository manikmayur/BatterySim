...
int runP2D(void)
{
    ...
    files[NSPECIES] = fopen("outP2D_Vcell.dat","w");
        ...
        void *user_data)
{
    ...
    for (size_t jx=0; jx < NX; jx++)
    {
        ...
        jloc = TWO*rateConst(jx,T)*std::sqrt(Ce*(dom.cLiMax-Cs)*Cs)*std::sinh(0.000005*F/(R*T)*(phiS-phiL-Ueq));
        ...
        sCe = (dom.domType==CA||dom.domType==AN)?dom.aLi*(1-p_tp)*jloc:ZERO;
