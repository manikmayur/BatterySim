...
static void SetInitialProfiles(N_Vector u, realtype dx)
{
	...
	for (jx=0; jx < MX; jx++) {
		...
		IJKth(udata,1,jx) = C1_SCALE*cx;
		IJKth(udata,2,jx) = C2_SCALE*cx;
	}
}
...
static int f(realtype t, N_Vector u, N_Vector udot, void *user_data)
{
	realtype q3, c1, c2, c1lt, c2lt;
	realtype c1rt, c2rt, hord1, hord2, horad1, horad2;
	realtype qq1, qq2, qq3, qq4, rkin1, rkin2, s;
	realtype q4coef, hordco, horaco;
	...
	for (jx=0; jx < MX; jx++) {
		...
		ileft = (jx == 0) ? 1 : -1;
		iright =(jx == MX-1) ? -1 : 1;
		c1lt = IJKth(udata,1,jx+ileft);
		c2lt = IJKth(udata,2,jx+ileft);
		c1rt = IJKth(udata,1,jx+iright);
		c2rt = IJKth(udata,2,jx+iright);
		...
		IJKth(dudata, 1, jx) =  hord1 + horad1 + rkin1;
		IJKth(dudata, 2, jx) = hord2 + horad2 + rkin2;
	}
